...
#### Setting up ----
ncore <- 4
cenacs <- shapefile(lcfg$path_cenblock_shape) # Using sp here, easier to work with raster
fl_hh <- raster(lcfg$path_hhdens_raster)
cenacs_wgs <- cenacs %>% spTransform(crs(fl_hh))

dat <- read_rds("synth/data/ipums.rds")
hh_dat <- dat %>%
