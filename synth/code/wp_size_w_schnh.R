...
## lehdwac
wac <- st_read(lcfg$path_wac_shape)
wac <- wac %>%
  st_drop_geometry()

## ncd
wp_coords <- fread(lcfg$path_workplace)
wp_coords <- wp_coords %>%
...
## NAICS
if (file.exists("synth/data/reusable/naics_emp_wpar.csv")) {
  naics <- fread("synth/data/reusable/naics_emp_wpar.csv")
...
naics_1712 <- fread(lcfg$path_naics_lookup)
...
wp_sf <- st_as_sf(wp_coords2, coords = c("x", "y")) %>%
  st_set_crs(4326)
